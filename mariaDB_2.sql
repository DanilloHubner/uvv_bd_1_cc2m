-- De início irei entrar com o banco de dados da UVV.
USE uvv;
-- Agora irei implementar o modelo elmarsi no MySQL.
--Primeiro criarei a tabela funcionario.
...
ALTER TABLE funcionario MODIFY COLUMN cpf_supervisor CHAR(11)  COMMENT 'Cpf do supervisor (FK da próxima tabela).'CHECK (cpf_supervisor != cpf);
...
-- Segundo, criarei a tabela departamento.
...
ALTER TABLE departamento MODIFY COLUMN numero_departamento INTEGER NOT NULL COMMENT 'Numero do departamento (PK desta tabela).';

ALTER TABLE departamento MODIFY COLUMN nome_departamento VARCHAR(15) NOT NULL COMMENT 'Nome do departamento.';

ALTER TABLE departamento MODIFY COLUMN cpf_gerente CHAR(11) NOT NULL COMMENT 'CPF do gerente do departamento (FK para a tabela funcionário).';

ALTER TABLE departamento MODIFY COLUMN data_inicio_gerente DATE COMMENT 'Data do início do gerente no departamento.';

-- Agora irei adicionar restrições de chave única ao atributo nome_departamento da tabela departamento.
...
ALTER TABLE localizacoes_departamento MODIFY COLUMN numero_departamento INTEGER NOT NULL COMMENT 'Número do departamento (Parte da PK desta tabela e também é uma FK para a tabela departamento).'CHECK (numero_departamento >= 0);

ALTER TABLE localizacoes_departamento MODIFY COLUMN local VARCHAR(15) NOT NULL COMMENT 'Localização do departamento (Parte da PK desta tabela).';
...
ALTER TABLE projeto MODIFY COLUMN numero_projeto INTEGER NOT NULL COMMENT 'Número do projeto (PK desta tabela).'CHECK (numero_projeto >= 0);
...
ALTER TABLE projeto MODIFY COLUMN numero_departamento INTEGER NOT NULL COMMENT 'É o número do departamento (FK para a tabela departamento).' CHECK (numero_departamento >= 0);
...
ALTER TABLE trabalha_em MODIFY COLUMN cpf_funcionario CHAR(11) NOT NULL COMMENT 'CPF do funcionário (Parte da PK desta tabela e é uma FK para a tabela funcionário).';

ALTER TABLE trabalha_em MODIFY COLUMN numero_projeto INTEGER  NOT NULL COMMENT 'É o número do projeto (Parte da PK desta tabela e é uma FK para a tabela projeto).'CHECK (numero_projeto >= 0);
...
ALTER TABLE dependente MODIFY COLUMN cpf_funcionario CHAR(11) NOT NULL COMMENT 'CPF do funcionário (Parte da PK desta tabela e é uma FK para a tabela funcionário).';

ALTER TABLE dependente MODIFY COLUMN nome_dependente VARCHAR(15) NOT NULL COMMENT 'Nome do dependente (Parte da PK desta tabela)';
...
-- Agora irei criar uma foreing key cpf_funcionário na tabela dependente e relacionando ao cpf da tabela funcionario.
